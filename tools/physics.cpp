...
glm::vec3 Object::nextPosOf(QTProfile* terrain) {
    ...
    glm::vec3 using_buffer_dataNormal = terrain->getResultNormalFromCoord(calcCoordFromPos(position + using_vertex_offset), using_vertex,  &using_buffer_data_index);
    ...
    if (glm::dot(facePoint_to_pos_vector, using_buffer_dataNormal) < 0.0f) {
        ...
        velocity += gravity * timeStep * glm::normalize(-position - using_vertex_offset);
        position += velocity * timeStep;
    }
    return position;
}
